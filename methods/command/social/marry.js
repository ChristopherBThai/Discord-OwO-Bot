...
module.exports = new CommandInterface({
	...
	execute: async function(p){
		...
		if(p.global.isUser(p.args[0])&&p.global.isInt(p.args[1])){
			...
			p.errorMsg(", invalid arguments! Please include the person you're marrying and the ring id.",3000);
			return;
		}
...
async function propose(p,user,ringId){
	...
	let embed = {
		...
		"timestamp":new Date(),
		"thumbnail":{
			"url":"https://cdn.discordapp.com/emojis/"+ring.emoji.match(/[0-9]+/)[0]+"."+(ringId>5?"gif":"png")
		}
	};
	p.send({embed});
}
...
async function upgradeRing(p,user,ringId,result,ringResult){
	// Validation checks
	if(!((p.msg.author.id==result.id1&&user.id==result.id2) || (p.msg.author.id==result.id2&&user.id==result.id1))){
	...
	let embed = {
		"description":"**"+p.msg.author.username+"**, are you sure you want to change your **"+currentRing.name+"** to a"+(p.global.isVowel(newRing.name)?"n":"")+" **"+newRing.name+"**?\n*You will not get your ring back!*",
		"thumbnail":{
			"url":"https://cdn.discordapp.com/emojis/"+currentRing.emoji.match(/[0-9]+/)[0]+"."+(currentRing.id>5?"gif":"png")
		},
		"color": p.config.embed_color
	}
	...
	collector.on('collect',async function(r){
		...
		if(r.emoji.name==yes){
			...
			embed.thumbnail.url = "https://cdn.discordapp.com/emojis/"+newRing.emoji.match(/[0-9]+/)[0]+"."+(newRing.id>5?"gif":"png")
			msg.edit({embed});
		}else{
			embed.color = 6381923;
...
async function display(p){
	...
	if(result.length<1){
		p.errorMsg(", you are not married! Buy a ring from `owo shop` and propose to someone!",3000);
		return;
	}
