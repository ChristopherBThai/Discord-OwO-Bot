...
async function display(p){
	...
	p.replyMsg(meatEmoji,", You currently have "+(count%6)+" meshi to feed Luffy! You now have "+Math.floor(count/6)+" badge(s) of "+luffyEmoji);
}
...
async function give(p,user){
	...
	if ( (result%6) - 2 < 0 ) {
		text += "\n"+p.config.emoji.blank+" **|** Wow! You have enough meshi to feed Luffy! "+luffyEmoji;
	}
