...
class LoveNote extends Collectible {
	constructor() {
		...
		this.displayMsg = '?emoji? **|** ?count? purple love notes <:purple_heart:1056420949911666778>'
		...
		this.giveMsg = '?emoji? **| ?receiver?**, take this **1** love note **<3**'
		...
		this.mergeMsg = '?mergeEmoji? | ?user?, your love notes turned into a love letter! I **<3** you';
	...
	async getGiveMsg(p, result, user, msgOverride) {
		if (p.msg.author.id === this.owners[0]) {
			...
			const giveMsg = '<:pink_letter:1056420948842135572> **| ?receiver?**, this **1** <:pink_letter:1056420948842135572> love note is for you **<3**'
	...
	async manualMerge(p) {
		...
		if (result1 == null || result1 < 0 || result2 == null || result2 < 0) {
			...
			p.send(`<a:warning:1056420952054960168> **| Seems like you don’t have enough love notes :c**`, 3000);
			p.setCooldown(5);
			return;
		}
