...
class Collectible {
  ...
  async display (p) {
	...
	async give (p, user) {
		...
		let result = await p.redis.hincrby(`data_${user.id}`, this.data, this.giveAmount);
		if (this.trackDate) {
	...
	async reset (p) {
		...
		await p.send(`⚙️ **| ${p.msg.author.username}**, I have reset the numbers for **${user.username}**`);
	}
