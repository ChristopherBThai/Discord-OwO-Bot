...
async function farmer(p) {
	...
	const msg = await p.send(`${rabbitEmoji} **|** I'll harvest 10 carrots for you, but I'm keeping half! Deal?`);
	let filter = (emoji,userID) => (emoji.name===yesEmoji||emoji.name===noEmoji)&&userID==p.msg.author.id;
