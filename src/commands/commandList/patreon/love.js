...
module.exports = new CommandInterface({
	...
	desc:"Give love to someone! Collect 5 to combine them into a heart master! You can only gain love if you receive it! This command was created by ?" + owner + "?",
...
async function display (p) {
	...
	p.replyMsg(loveEmoji,", You currently have "+(count%5)+" love and "+Math.floor(count/5)+" "+heartMasterEmoji+" heart master(s)!");
}
...
async function give(p,user){
	...
	if ((result % 5) - 2 < 0) {
		text += "\n" + p.config.emoji.blank + " **|** Your 5 hearts fused into a heart master! " + heartMasterEmoji;
	}
