...
module.exports = new CommandInterface({
	...
	desc:"Give a snowball to someone! Collect 5 to combine them into a snowman! You can only gain snowballs if you receive it! This command was created by ?" + owner + "?",
...
async function display(p) {
	...
	p.replyMsg(snowballEmoji, `, you currently have ${snowballCount} ${snowballEmoji} ${snowballString} and ${snowmanCount} ${snowmanEmoji} ${snowmanString}!`);
}
...
async function give(p, user) {
	...
	if ((result % 5) - 2 < 0) {
		text += "\n" + p.config.emoji.blank + " **|** Your 5 snowballs combined into a snowman " + snowmanEmoji;
	}
