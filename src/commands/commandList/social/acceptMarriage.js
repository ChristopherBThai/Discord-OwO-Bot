	...
	execute: async function(p){
		...
		if(result[0].length>0&&result[1].affectedRows>0){
			...
			if(!sender){
				...
				let heart = [hearts[Math.trunc(Math.random()*hearts.length)],hearts[Math.trunc(Math.random()*hearts.length)],hearts[Math.trunc(Math.random()*hearts.length)]];
				p.replyMsg(ring.emoji," and **"+sender.username+"** are now married! Congratulations!! "+heart.join(" "));
			}
		}else{
