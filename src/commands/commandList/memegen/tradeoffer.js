...
module.exports = new CommandInterface({
	...
	execute: function(p){
		...
		if(args.length < 2 || args.length > 3) {
			p.errorMsg(", the correct arguments are `{leftText|@user|emoji} | {rightText|@user|emoji} | {@user|emoji}`",3000);
			p.setCooldown(5);
			return;
		}
		let hasThird = false;
		...
		canvasUtil.loadBackground('./src/data/images/tradeoffer.jpg',function(err, ctx, canvas, image){
			if(err){
				p.send("**🚫 | "+p.msg.author.username+"**, Uh oh.. this command is broken!", 3000);
				return
			}

			let textArgs = {x:20,y:300,width:240,height:220,size:30,stroke:3,color:'black',text:args[0],imageSize:120,align:'center',ifImage:{y:-50}, imageFontSize: 35 };
