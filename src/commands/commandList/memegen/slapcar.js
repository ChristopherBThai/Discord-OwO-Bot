...
module.exports = new CommandInterface({
	...
	example: [
		'owo slapcar This badboy can fit so much coffee @Scuttler',
		'owo slapcar I slap cars',
	],
	...
	execute: function (p) {
		if (p.global.isUser(p.args[p.args.length - 1])) user(p);
			...
			/^\s*<a?:[a-zA-Z0-9]+:[0-9]+>\s*$/gi.test(p.args[p.args.length - 1])
		)
			emoji(p);
		else car(p);
	},
...
function car(p) {
	fs.readFile('./src/data/images/slap_car.jpg', function (err, image) {
		...
		if (lines > 4) {
			p.send(
				'**🚫 | ' + p.msg.author.username + '**, The text is too long!',
				3000
			);
			p.setCooldown(5);
			return;
...
function user(p) {
	fs.readFile(
		'./src/data/images/slap_transparent.png',
					...
					if (!err && response.statusCode == 200) {
						...
						img2.onload = function () {
							...
							for (var i = 0; i < tempText.length; i++) {
								if (
									ctx.measureText(text + tempText[i] + ' ').width > 700 &&
									i > 0
								)
									text += '\n';
								text += tempText[i] + ' ';
