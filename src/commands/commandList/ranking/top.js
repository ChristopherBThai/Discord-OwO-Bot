...
module.exports = new CommandInterface({
	...
	args: 'points|guild|zoo|money|cookie|pet|huntbot|luck|curse|battle|daily|level|shard|marriage [global] {count}',
...
function getRanking(globalRank, con, msg, count, p) {
	...
	displayRanking(
		...
		'Top ' +
			...
			(globalRank
				? 'Global OwO Rankings'
				: 'OwO Rankings for ' + msg.channel.guild.name),
		function (query, rank) {
			if (rank == 0)
				return (
					'>\t\tyou said owo ' + global.toFancyNum(query.count) + ' times!\n\n'
				);
			else
...
function getZooRanking(globalRank, con, msg, count, p) {
	...
	displayRanking(
		...
		'Top ' +
			...
			(globalRank
				? 'Global Zoo Rankings'
				: 'Zoo Rankings for ' + msg.channel.guild.name),
		function (query, rank) {
			if (rank == 0)
				return (
					'>\t\t' +
...
function getMoneyRanking(globalRank, con, msg, count, p) {
	...
	displayRanking(
		...
		'Top ' +
			...
			' ' +
			(globalRank
				? 'Global Cowoncy Rankings'
				: 'Cowoncy Rankings for ' + msg.channel.guild.name),
		function (query, rank) {
			if (rank == 0)
...
function getRepRanking(globalRank, con, msg, count, p) {
	...
	displayRanking(
		...
		'Top ' +
			...
			' ' +
			(globalRank
				? 'Global Cookie Rankings'
				: 'Cookie Rankings for ' + msg.channel.guild.name),
		function (query, rank) {
			if (rank == 0)
...
function getPetRanking(globalRank, con, msg, count, p) {
	...
	displayRanking(
		...
		'Top ' +
			...
			(globalRank
				? 'Global Pet Rankings'
				: 'Pet Rankings for ' + msg.channel.guild.name),
		function (query, rank) {
			let result = '\t\t ';
...
function getHuntbotRanking(globalRank, con, msg, count, p) {
	...
	if (globalRank) {
		sql =
			'SELECT id,total FROM autohunt ORDER BY total DESC LIMIT ' + count + ';';
		sql +=
	...
	displayRanking(
		...
		'Top ' +
			...
			' ' +
			(globalRank
				? 'Global HuntBot Rankings'
				: 'HuntBot Rankings for ' + msg.channel.guild.name),
		function (query, rank) {
			if (rank == 0)
...
function getLuckRanking(globalRank, con, msg, count, p) {
	...
	if (globalRank) {
		...
		let users = global.getids(msg.channel.guild.members);
		sql =
			'SELECT * FROM luck WHERE id IN (' +
			users +
			') ORDER BY lcount DESC LIMIT ' +
			count +
			';';
		sql +=
	...
	displayRanking(
		...
		'Top ' +
			...
			' ' +
			(globalRank
				? 'Global Luck Rankings'
				: 'Luck Rankings for ' + msg.channel.guild.name),
		function (query, rank) {
			if (rank == 0)
...
function getCurseRanking(globalRank, con, msg, count, p) {
	...
	displayRanking(
		...
		'Top ' +
			...
			' ' +
			(globalRank
				? 'Global Curse Rankings'
				: 'Curse Rankings for ' + msg.channel.guild.name),
		function (query, rank) {
			if (rank == 0)
				return '>\t\tLuck: ' + global.toFancyNum(query.lcount) + '\n\n';
			else return '\n\t\tLuck: ' + global.toFancyNum(query.lcount) + '\n';
	...
	con.query(sql, async function (err, rows, fields) {
		...
		if (rows[1][0] !== undefined && rows[1][0] !== null) {
			...
			embed +=
				...
				global.toFancyNum(rows[1][0].count) +
				' times!\n\n';
		}
		for (let ele of rows[0]) {
			let id = String(ele.id);
...
function getDailyRanking(globalRank, con, msg, count, p) {
	...
	if (globalRank) {
		sql =
			'SELECT * FROM cowoncy ORDER BY daily_streak DESC LIMIT ' + count + ';';
		sql +=
...
function getMarriageRanking(globalRank, con, msg, count, p) {
	...
	if (globalRank) {
		sql =
			'SELECT uid1 AS id,uid2 AS id2,dailies FROM marriage ORDER BY dailies DESC LIMIT ' +
			count +
			';';
		sql +=
			'SELECT uid1 AS id,uid2 AS id2,dailies,(SELECT COUNT(*)+1 FROM marriage WHERE dailies > u.dailies ) AS rank FROM marriage u WHERE u.uid1 = ' +
			msg.author.id +
			' OR u.uid2 = ' +
			msg.author.id +
			';';
		...
		let users = global.getids(msg.channel.guild.members);
		sql =
			'SELECT uid1 AS id,uid2 AS id2,dailies FROM marriage WHERE uid1 IN (' +
			users +
			') OR uid2 IN (' +
			users +
			') ORDER BY dailies DESC LIMIT ' +
			count +
			';';
		sql +=
			'SELECT uid1 AS id,uid2 AS id2,dailies,(SELECT COUNT(*)+1 FROM marriage WHERE (uid1 IN (' +
			users +
			') OR uid2 IN (' +
			users +
			')) AND dailies > u.dailies ) AS rank FROM marriage u WHERE u.uid1 = ' +
			msg.author.id +
			' OR u.uid2 = ' +
			msg.author.id +
			';';
	...
	displayRanking(
		...
		'Top ' +
			...
			' ' +
			(globalRank
				? 'Global Marriage Rankings'
				: 'Marriage Rankings for ' + msg.channel.guild.name),
		function (query, rank) {
			if (rank == 0)
				return (
					'>\tDailies Collected Together: ' +
					global.toFancyNum(query.dailies) +
					'\n\n'
				);
			else
				return (
