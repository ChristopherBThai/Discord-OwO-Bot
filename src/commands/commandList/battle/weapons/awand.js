...
module.exports = class AbsorbWand extends WeaponInterface {
	...
	attackWeapon(me, team, enemy) {
		...
		let replenishLogs;
		if (lowestWp) replenishLogs = WeaponInterface.replenish(lowestWp, stole, me, { me, allies: team, enemies: enemy });

		/* Add everything to logs */
		if (!lowestWp||!replenishLogs) logs.push(`[AWAND] ${me.nickname} damaged ${attacking.nickname} for ${damage.amount} HP`);
		else logs.push(`[AWAND] ${me.nickname} damaged ${attacking.nickname} for ${damage.amount} HP and transfered ${Math.round(stole)} WP to ${lowestWp.nickname}`);
