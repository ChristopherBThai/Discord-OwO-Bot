...
const weaponUtil = require('./util/weaponUtil');
const maxBoxes = 50;
...
async function openCrate(p, count = 1) {
	...
	if (count == 1) {
		...
		text1 = stripIndents`
			${p.config.emoji.blank} **| "+p.msg.author.username+"** opens a weapon crate
		...
		text1 = stripIndents`
			${p.config.emoji.blank} **| ${p.msg.author.username}** opens ${count} weapon crates
