...
const battleImageUtil = require('../battleImage');
const WeaponInterface = require('../WeaponInterface');
var allBuffs = WeaponInterface.allBuffs;
const crateUtil = require('./crateUtil');
const alterBattle = require('../../patreon/alterBattle');
const { GEN_HOST, GEN_API_HOST, GEN_PASS } = process.env;
const maxAnimals = 6;
const attack = '👊🏼';
...
exports.displayAllBattles = async function (p, battle, logs, setting) {
	...
	if (setting.showLogs == 'link') {
		...
		if (uuid) logLink = `${GEN_HOST}/battle-log/${uuid}`;
	}
