...
async function addQuest (p) {
	/* Query for user info */
	let sql = "SELECT questTime FROM timers WHERE uid = (SELECT uid FROM user WHERE id = "+p.msg.author.id+");";
	sql += "SELECT * FROM quest WHERE uid = (SELECT uid FROM user WHERE id = "+p.msg.author.id+") ORDER BY qid asc;";
...
function parseQuest(questInfo){
	...
	if (questInfo.prize == "cowoncy" ) {
		...
		reward = global.toFancyNum(quest.shards[questInfo.level]) + " <:weaponshard:655902978712272917>";
	}
