...
exports.use = async function (p, ids) {
	...
	if (ids.length === 1 ) {
		if (!results || results.changedRows == 0) {
			...
			text = `**${gems[0].emoji} | ${p.msg.author.username}**, you activated a(n) **${ranks[gems[0].key[0]]} ${gems[0].type} Gem**!\n`
...
exports.desc = async function(p, id){
	...
	const sql = "SELECT * FROM user NATURAL JOIN user_gem NATURAL JOIN gem WHERE id = "+p.msg.author.id+" AND gname = '"+gem.key+"';";
