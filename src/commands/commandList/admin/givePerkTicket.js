...
async function giveTicket(p, id, count = 1, type = 1) {
	...
	if(count>0)
		...
		user = await p.sender.msgUser(id, `${emoji} **|** Sorry about that. You have lost **${Math.abs(count)} ${emoji} ${name}**.`);

	if(user && !user.dmError)
		return { user, count}
	else
		await p.errorMsg(', Failed to message user for '+id);
}
