...
const patreon = require('../../../botHandlers/patreonHandler');
const cowoncy = [
	'184587051943985152',
	'184587051943985152',
	'184587051943985152',
...
async function getPatreons(p) {
	...
	if (patreons.pet.length) {
		...
		for (let i in list) {
			...
			csv += `${user ? user.username : 'A User'}, ${list[i].discord}, ${
				list[i].name
			}\n`;
		}
	}
	...
	await p.send(
		`Type \`owo distributecowoncy {amount}\` to send monthly cowoncy to ${patreons.cowoncy.length} + ${result.length} users`
	);
...
async function distributeCowoncy(p) {
	...
	let text = stripIndents`
		Distributed ${amount} cowoncy to ${cowoncy.length} users
		\```json
