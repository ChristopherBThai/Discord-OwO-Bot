...
module.exports = new CommandInterface({
	...
	execute: async function () {
		...
		if (!message || message === '') {
			...
			this.errorMsg(', you can only create suggests from our support server! Join us at ' + this.config.guildlink);
			return this.setCooldown(5);
		}
...
async function suggest (message) {
	...
	const filter = (componentName, user) => [interactionAgree, interactionDisagree].includes(componentName) && user.id === this.msg.author.id;
	const collector = this.interactionCollector.create(msg, filter, { time: 900000 });
	...
	collector.on('collect', async (componentName, user, ack) => {
		...
		if (componentName === interactionAgree) {
			...
			if (result) {
				...
				embed.footer = { text: check + " The suggestion has been posted! Thank you!" };
			}
		} else if(componentName === interactionDisagree) {
			embed.footer = { text: cross + " You decided not to post the suggestion!" };
