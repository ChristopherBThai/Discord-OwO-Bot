...
module.exports = new CommandInterface({
	...
	execute: async function(p){
		let con=p.con,client=p.client,msg=p.msg;
		...
		let totalAnimals = parseInt(rows[1][0].common)+parseInt(rows[1][0].uncommon)+parseInt(rows[1][0].rare)+parseInt(rows[1][0].epic)+parseInt(rows[1][0].mythical)+parseInt(rows[1][0].legendary);
		let disabled = "";
		for(i in rows[2]){
