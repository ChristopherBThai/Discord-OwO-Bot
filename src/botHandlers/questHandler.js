...
const mysql = require('./mysqlHandler');
...
module.exports = class Quest {
	...
	async increment(msg, questName, count = 1, extra) {
		...
		if (!result[0]) return;
		for (let i = 0; i < result.length; i++) await check(msg, id, username, questName, result[i], count, extra);
	}
};
...
async function check(msg, id, username, questName, result, count, extra) {
	...
	let text, rewardSql, variables, rewardVar;
	if (current >= needed) {
